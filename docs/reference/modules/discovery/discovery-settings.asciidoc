...
`cluster.auto_shrink_voting_configuration`::
    ...
    Specifies whether the <<voting-configurations,voting configuration>> shrinks
    automatically. The default value is `true`, which means the voting
    configuration automatically sheds departed nodes as long as it still
    contains at least 3 nodes.  If set to `false`, the voting configuration
    never shrinks automatically; departed nodes must be removed manually using
    the
...
`discovery.zen.hosts_provider`:: 
    Specifies which type of <<discovery-hosts-providers,hosts provider>> provides
