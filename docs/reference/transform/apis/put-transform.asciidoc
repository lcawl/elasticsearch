...
You must provide either a `pivot` object or a `latest` object.
...
`latest`::
(Required^*^, object)
include::{es-repo-dir}/rest-api/common-parms.asciidoc[tag=latest]
+
...
`sort`:::
(Required, string)
include::{es-repo-dir}/rest-api/common-parms.asciidoc[tag=latest-sort]

`unique_key`:::
(Required, string)
include::{es-repo-dir}/rest-api/common-parms.asciidoc[tag=latest-key]
